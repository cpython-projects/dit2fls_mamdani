...
words_4 = {
    ...
    'words': {
        'Unimportant': {'umf': ('trapmf', 0, 0, 0.55, 4.61), 'lmf': ('trapmf', 0, 0, 0.09, 1.15, 1)},
        'More or less unimportant': {'umf': ('trapmf', 0.42, 2.25, 4.00, 5.41), 'lmf': ('trapmf', 2.79, 3.21, 3.21, 0.34, 3.71)},
        'More or less important': {'umf': ('trapmf', 3.38, 5.50, 7.25, 9.02), 'lmf': ('trapmf', 5.79, 6.28, 6.28, 0.33, 6.67)},
        'Very important': {'umf': ('trapmf', 7.37, 9.36, 10, 10), 'lmf': ('trapmf', 8.68, 9.91, 10, 10, 1)},
    }
}


def preprocessing(words_model):
    words_model['U'] = np.arange(*words_model['X'])

    # Creation of the MFs
    for word, value in words_model['words'].items():
        umf_type, *umf_params = value['umf']
        lmf_type, *lmf_params = value['lmf']
        words_model['words'][word]['range'] = (min(umf_params), max(umf_params))

        words_model['words'][word]['umf'] = getattr(t1_membership_functions, umf_type)(words_model['U'], *umf_params)
        words_model['words'][word]['lmf'] = getattr(t1_membership_functions, lmf_type)(words_model['U'], *lmf_params)
